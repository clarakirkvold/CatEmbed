...
####################################################################################
#                                                                                  #
# This is script is a slightly modified version of the CategoricalEmbedder package #
# developed by Shivanand Roy and Prakash Behera                                    # 
# which is available at https://github.com/Shivanandroy/CategoricalEmbedder.git    #
#                                                                                  #
####################################################################################
# Necessary imports:
...
from tensorflow.keras.layers import Dense, Dropout, Embedding, Activation, Input, concatenate, Reshape, Flatten
...
def get_label_encoded_data(data, categorical_variables=None):
    ...
    labels = {}  # Dictionary to store encoded labels

    for var in categorical_variables:
        encoders[var]=LabelEncoder()

        df.loc[:, var] = encoders[var].fit_transform(df[var])
        labels[var] = dict(zip(encoders[var].classes_, encoders[var].transform(encoders[var].classes_)))
        d=encoders[var].fit(df[var])
        encoded_labels=encoders[var].classes_
        encoded_labels=d.transform(encoded_labels)
        encoders[var]=encoded_labels


    return df, encoders, labels
...
def get_embeddings(X_train, y_train, categorical_embedding_info, is_classification, epochs=100, batch_size=256):
    ...
    for col in categorical_embedding_info.keys():
        x_inputs.append(X_train[col].values.astype(float))
    ...
    nnet.fit(x_inputs, y_train.values, batch_size=batch_size, epochs=epochs, validation_split=0.2, verbose=1)
...
def get_embeddings_in_dataframe(embeddings, encoders):
    ...
    for cat_var in tqdm(embeddings.keys()):
        ...
        data[cat_var] = data[cat_var].astype(str) # convert integer column to string
        print('df=',df)
